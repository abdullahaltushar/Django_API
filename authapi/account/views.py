...
from account.seializers import UserRegistrationSerializer,UserLoginSerializer, UserProfileSerializer, UserChangePasswordSerializer, SendPasswordResetEmailSerializer, UserPasswordResetSerializer
...
class UserChangePasswordView(APIView):
    ...
    def post(self, request, format=None):
        ...
        print(serializer)
        if serializer.is_valid(raise_exception=True):
            return Response({'msg':'Password change succefully'},status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
